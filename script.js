...
(function () {
  ...
  function buildInvitation(c, dateInfo, timeText) {
	...
	const motherPart = motherName ? ` <span class="dot"></span> <span${motherDec}>${motherName}</span>` : '';
  
  return `<span${fatherDec}>${fatherName}</span>${motherPart}<span style="color:#999;margin-left:4px">의 ${side === c.groom ? '아들' : '딸'}</span> <strong>${side.name}</strong>`;
}
      parents.innerHTML = `
